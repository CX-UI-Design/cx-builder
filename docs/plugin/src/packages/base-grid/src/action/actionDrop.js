...
export default {
  ...
  render(h) {
    return (
      <div
        id={'header-drag'}
        class={{ opened: this.headSetSw.set }}
        on-click={$event => this.dragClick($event)}
      >
        <draggable
          class="header-drag_content"
          v-model={this.dropList}
        ...
        >
          {this.dropList.map((item, $index) => [
            <li>
              <el-checkbox class="header-drag_checkbox fl" v-model={item[this.headRefer['hidden']]}>
        ...
        <div class="header-drag_submit">
          <el-button type={'primary'} size={'small'} on-click={this.setHeadSubmit}>
            确定
          </el-button>
        </div>
      </div>
