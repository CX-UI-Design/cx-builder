  ...
  >
    ...
    <template>
      <div class="grid-container" v-if="isRender">
        ...
        <el-table
          :class="{ 'top-gap': firstRowHasError.value }"
          :data="actualGridData.list"
          :border="border"
          :max-height="sizeInfo.maxHeight"
        ...
        >
          ...
          <el-table-column
            v-if="firstColRender"
            :type="firstColInfo[headRefer['col-type']]"
            :label="firstColInfo[headRefer['label']]"
            :width="firstColInfo[headRefer['width']]"
            align="center"
            :fixed="true"
          ...
          <!-- show radio -column -->
          <el-table-column
            v-if="firstColInfo[headRefer['col-type']] === 'radio'"
            :label="firstColInfo[headRefer['label']]"
            :width="firstColInfo[headRefer['width']]"
            align="center"
            :fixed="true"
          ...
          >
            <template slot-scope="scope">
              <el-radio
                v-model="radioModel"
                :label="scope.$index"
                @change.native="selectionChange(scope.row, scope.$index)"
                >{{ null }}</el-radio
              >
            </template>
          </el-table-column>
          <!-- normal-column -->
          <el-table-column
            v-if="!item[headRefer['hidden']]"
            v-for="(item, index) in gridHead"
            :index="index"
            :key="index"
            :class-name="normalColClassName(item)"
            :min-width="item[headRefer['width']]"
            :align="align"
            :label="item[headRefer['label']]"
            :resizable="resizable"
            sortable="custom"
          ...
          >
            ...
            <template slot-scope="scope">
              ...
              <slot-scope
                ...
                :colIndex="index"
                :firstRowHasError="firstRowHasError"
                :gridCheck="gridCheck"
                :topGapForErrMsg="topGapForErrMsg"
                :wrapperHeight="wrapperHeight"
                @cell-action="cellAction"
                @set-formCell-check="setFormCellCheck"
              ></slot-scope>
            </template>
          </el-table-column>
          ...
          <el-table-column
            v-if="actionRender"
            :label="ationColInfo[headRefer['label']]"
            :width="ationColInfo[headRefer['width']]"
            fixed="right"
            :align="ationColInfo[headRefer['align']]"
...
import actionScope from './action/actionScope';
import slotScope from './slot-scope/slot-scope';
import actionPanel from './action/actionPanel';
import errorPrompt from './error-unit/error-prompt';

// import {actionSummary, actionDrop, actionScope, slotScope, actionPanel, errorPrompt} from './ComReg'
...
export default {
  ...
  computed: {
    ...
    addRowColRender() {
      return (
        this.showAddRowOperation && (this.errorType === 'noError' || this.errorType === 'noResult')
      ); //开关开启 + 只要表头/表数据正常即可
    },
    //表头设置列渲染开关
    headOperationColRender() {
      return (
        this.showHeadOperation && (this.errorType === 'noError' || this.errorType === 'noResult')
      ); //开关开启 + 只要表头/表数据正常即可
    },
    //首列的配置
    firstColInfo() {
